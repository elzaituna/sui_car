module store_management::store_management {
    ...
    const EUnauthorizedAccess: u64 = 9;
    const EPartialPayment: u64 = 10;
    ...
    struct Transaction has key, store {
        ...
        partial_payments: Balance<SUI>,   // Balance for partial payments
    }
    ...
    fun reset_transaction(transaction: &mut Transaction) {
    ...
    /// New feature: Adds a partial payment to the transaction.
    public entry fun add_partial_payment(transaction: &mut Transaction, amount: Coin<SUI>, ctx: &mut TxContext) {
        assert!(tx_context::sender(ctx) == transaction.customer, ENotStore);
        let partial_payment = coin::into_balance(amount);
        balance::join(&mut transaction.partial_payments, partial_payment);
    }

    /// New feature: Completes a partial payment transaction.
    public entry fun complete_partial_payment(transaction: &mut Transaction, ctx: &mut TxContext) {
        assert!(transaction.customer == tx_context::sender(ctx), EUnauthorizedAccess);
        let partial_payment_amount = balance::value(&transaction.partial_payments);
        let partial_payment_coin = coin::take(&mut transaction.partial_payments, partial_payment_amount, ctx);
        balance::join(&mut transaction.escrow, partial_payment_coin);
        // Check if total escrow balance meets or exceeds the transaction price
        let total_balance = balance::value(&transaction.escrow);
        if total_balance >= transaction.price {
            transaction.transactionFulfilled = true;
        }
    }
}
